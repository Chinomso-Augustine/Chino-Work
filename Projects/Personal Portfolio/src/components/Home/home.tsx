...
function Home() {
  ...
  const Images = Object.entries(SamplePics).map(([name, img]) => ({
    value: name,
    img,
  }));
  ...
  const aboutMe = [
    ...
    {
      value: "Athlete of The Month Featured On Spectrum News",
      img: News,
    },
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-10 max-w-6xl mx-auto">
          {aboutMe.map((events, index) => (
            ...
            >
              ...
              <img
                src={events.img}
                className={`w-full h-64 object-cover rounded-t-xl py-4`}
                alt={events.value}
              />
            </div>
          ))}
